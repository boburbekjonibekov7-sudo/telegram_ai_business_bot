    ...
    async def handle_callback_query(self, callback: dict[str, Any]) -> None:
        ...
        if data.startswith(("settings:edit:", "settings:delete:")):
            ...
            if len(parts) == 3 and field in {"toggle", "time"}:
                ...
                enabled = self._toggle_user_setting_with_default(user_id, key, default)
                if field == "toggle":
...
🟢 Buyruqlar ruxsati — qaysi buyruqni kim ishlatishini belgilaysiz."""
    ...
    def _toggle_user_setting(self, owner_id: int, key: str) -> bool:
        return self._toggle_user_setting_with_default(owner_id, key, "0")

    def _toggle_user_setting_with_default(self, owner_id: int, key: str, default: str) -> bool:
        current = self._user_setting(owner_id, key, default)
